...
- [HerpAI-Ingestion: Solution Design Document](#herpai-ingestion-solution-design-document)
  ...
  - [Scheduler CLI Commands](#scheduler-cli-commands)
    ...
    - [Connector Data Models](#connector-data-models-1)
    - [Connector Utilities](#connector-utilities-1)
    - [Connector Exceptions](#connector-exceptions-1)
    - [Data Flow](#data-flow-1)
    - [Component Interactions](#component-interactions-1)
  - [Sample Connector Implementation](#sample-connector-implementation-1)
    - [Connector Registry](#connector-registry-1)
    - [Connector Specifications](#connector-specifications-1)
  - [Data Processing Pipeline](#data-processing-pipeline-1)
    - [Extraction Phase](#extraction-phase-1)
    - [Transformation Phase](#transformation-phase-1)
    - [Loading Phase](#loading-phase-1)
  - [Data Model and Repository Pattern](#data-model-and-repository-pattern-1)
    - [Domain Entities](#domain-entities-1)
    - [Repository Interfaces](#repository-interfaces-1)
    - [Specifications](#specifications-1)
    - [Repository Implementations](#repository-implementations-1)
    - [Scheduler Repository](#scheduler-repository)
...
HerpAI-Ingestion incorporates a flexible scheduling system that enables runtime modification of ingestion schedules through both the CLI and API interfaces.

### Scheduling Domain Model

```python
# herpai_ingestion/domain/scheduled_job.py
from openbiocure_corelib.data.entity import BaseEntity
...
class ScheduledJob(BaseEntity):
    """Entity representing a scheduled ingestion job."""
    ...
    source: Mapped[str] = mapped_column(nullable=False)
    query: Mapped[str] = mapped_column(nullable=False)
    schedule: Mapped[str] = mapped_column(nullable=False)  # Cron expression
...
```python
# herpai_ingestion/repository/interfaces/scheduled_job_repository.py
from typing import Protocol, List, Optional
from openbiocure_corelib.data.repository import IRepository
from herpai_ingestion.domain.scheduled_job import ScheduledJob
...
class IScheduledJobRepository(IRepository[ScheduledJob], Protocol):
    ...
    async def find_due(self, current_time: datetime) -> List[ScheduledJob]: ...
```

### Scheduler Service

```python
# herpai_ingestion/services/scheduler_service.py
import asyncio
import logging
import uuid
from datetime import datetime, UTC
...
from openbiocure_corelib import engine
from herpai_ingestion.domain.scheduled_job import ScheduledJob
from herpai_ingestion.repository.interfaces.scheduled_job_repository import IScheduledJobRepository
from herpai_ingestion.services.ingestion_service import IngestionService
...
class SchedulerService:
    ...
    async def get_jobs(self) -> List[ScheduledJob]:
        ...
        job_repository = engine.resolve(IScheduledJobRepository)
        return await job_repository.get_all()
    
    async def get_job(self, job_id: str) -> Optional[ScheduledJob]:
        """Get a scheduled job by ID."""
        job_repository = engine.resolve(IScheduledJobRepository)
        return await job_repository.get_by_id(job_id)
    
    async def get_job_by_name(self, name: str) -> Optional[ScheduledJob]:
        """Get a scheduled job by name."""
        job_repository = engine.resolve(IScheduledJobRepository)
        return await job_repository.find_by_name(name)
    ...
    async def create_job(self, job_data: Dict[str, Any]) -> ScheduledJob:
        ...
        existing_job = await job_repository.find_by_name(job_data["name"])
        if existing_job:
    ...
    async def update_job(self, job_id: str, job_data: Dict[str, Any]) -> ScheduledJob:
        """Update an existing scheduled job."""
        job_repository = engine.resolve(IScheduledJobRepository)
        ...
        if "name" in job_data and job_data["name"] != job.name:
            existing_job = await job_repository.find_by_name(job_data["name"])
            if existing_job:
                raise ValueError(f"Job with name '{job_data['name']}' already exists")
        ...
        if "query" in job_data:
            job.query = job_data["query"]
        ...
        # Save updated job
        return await job_repository.update(job)
    
    async def delete_job(self, job_id: str) -> bool:
        """Delete a scheduled job."""
        job_repository = engine.resolve(IScheduledJobRepository)
        return await job_repository.delete(job_id)
    ...
    async def _scheduler_loop(self):
        ...
        while self._running:
            try:
                ...
                # Find due jobs
                due_jobs = await job_repository.find_due(current_time)
                ...
                for job in due_jobs:
                    ...
                    # Update job with last run time and next run time
    ...
    async def _execute_job(self, job: ScheduledJob):
        ...
        try:
            # Resolve ingestion service
            ingestion_service = engine.resolve(IngestionService)
            
            # Execute ingestion
            result = await ingestion_service.ingest(job.source, job.query, job.limit)
            
            logger.info(f"Scheduled job completed: {job.name} - processed {result['documents_processed']} documents")
            
        except Exception as e:
            logger.error(f"Error executing scheduled job {job.name}: {str(e)}")
```
```
